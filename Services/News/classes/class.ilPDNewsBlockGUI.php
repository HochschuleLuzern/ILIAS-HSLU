...
class ilPDNewsBlockGUI extends ilNewsForContextBlockGUI
{
    ...
    public function __construct()
    {
        ...
        if ($this->getDynamic() && !$this->cache_hit) {
            $this->dynamic = true;
            $data = array();
        } elseif ($this->getCurrentDetailLevel() > 0) {
...
// BEGIN PATCH HSLU: Add links to the titles of the enabled desktop-boxes
        $news_link = $DIC->ctrl()->getLinkTargetByClass('ilPersonalDesktopGUI', 'jumpToNews');
...
// END PATCH HSLU: Add links to the titles of the enabled desktop-boxes        
    ...
    public function getHTML()
    {
        ...
        if ($enable_internal_rss) {
            ...
            $this->addBlockCommand(
                ...
                true,
                false,
                ilRSSButtonGUI::get(ilRSSButtonGUI::ICON_RSS)
            );
        }
    ...
    */
    ...
    public function addCloseCommand($a_content_block)
    {
        $lng = $this->lng;
        $ilCtrl = $this->ctrl;
        
        $a_content_block->addHeaderCommand(
            $ilCtrl->getParentReturn($this),
            $lng->txt("selected_items_back")
        );
    }
